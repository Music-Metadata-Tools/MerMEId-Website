...
When a user clicks a link inside a `shacl-form` field (links are rendered as `<a class="uuid">`), the editor intercepts the click using `composedPath()` to pierce the shadow DOM, reads the linked `.ttl` file from the virtual filesystem, and renders it in a floating, draggable, resizable panel using a second `shacl-form` instance in read-only `data-view` mode.
...
- Fetch pre-built dataset index files (one `.ttl` per entity type) from `datasetBaseUrl`
- Parse all index files into an in-memory Oxigraph RDF store
- Run a SPARQL query to extract all `skos:prefLabel` values and their types
- Filter results by label text and/or entity type
- On selection, navigate the filesystem manager tree to the matching file

### How Indexes Work

The search component does **not** index files from the local virtual filesystem. Instead, it fetches pre-built aggregate index files from the `datasetBaseUrl` — a published static URL, typically GitHub/GitLab Pages. These files are generated by a CI/CD pipeline when changes are pushed to the repository.
